...
+(function (source, $) {
    ...
    */
    var mvapiUrl = {
        //apiurl: 'https://mediavaletappsapi-dev.azurewebsites.net/api/mediavalet'
        //apiurl: 'http://zaidmicrosoft.azurewebsites.net/api/mediavalet'
        //apiurl: 'http://videoneticsvms.cloudapp.net/mediavalet/api/MediaValetAppsApis/Mvapi/api/mediavalet'
        //apiurl: 'https://api-usil.mediavalet.net'
        apiurl: 'https://mediavaletappsapi-dev.azurewebsites.net/api/mediavalet'
    }
    /**Api EndPouints
    */
   ...
   */
    var mediaValet = function () {
        ...
        */
        ...
        mediaValet.prototype.newGuid = function guid() {
            function s4() {
        ...
        */
        mediaValet.prototype.urlmapping = function (username, appname) { //, application (new code)
            var d = $.Deferred();
            var jsonobject = new Object();
            var emailDomain = username;
            var data = {
                emailDomain: emailDomain
            };
            var url = mvapiUrl.apiurl + mvapiEndpoints.GetDomainData + "?email=" + emailDomain + "" + "&appsname=" + appname;
            var request = $.ajax({
                url: url,
                async: true
            });
       ...
       */
        ...
        */
        mediaValet.prototype.GetAssets = function (filter, token, offset, count, appsname) {
            ...
            //}
            if (appsname == 'hootsuite' && filter == undefined) {
                filter = "(AssetType+EQ+Image+OR+AssetType+EQ+File)";
            }
            if (filter == undefined) {
        ...
        */
        ...
        mediaValet.prototype.ADSettings = function () {
            try {
                var deferredobj = $.Deferred();
                var tenantid = "c2ccdaa6-99e8-4789-a22e-a634d94b93e4";
                var clientid = "7d578d4b-b655-430d-99a9-91dff068f13d";
                var redircturi = "https://mvo365demo.mediavalet.com/githubmv/office.html";
                var state = mediaValet.prototype._guid();
                var nounce = mediaValet.prototype._guid();
                var AdSettings = [
                { "clientid": clientid, "tenantid": tenantid, "redircturi": redircturi, "state": state, "nonce": nounce },
                ];
                $.when(AdSettings).done(function (data) {
                    deferredobj.resolve(data);
                }).fail(function (fail) {
                    deferredobj.resolve(fail);
                });
            } catch (e) {
                deferredobj.resolve(e.message);
            }
            return deferredobj.promise();
        }
        mediaValet.prototype._guid = function () {
        ...
        mediaValet.prototype.AzureTokenVarification = function () {
